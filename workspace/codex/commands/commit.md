...
Dev:
  M .gitignore
  M workspace/codex/commands/run.md
```
...
workspace: {hash} ({N} files)
Dev: {hash} ({N} files) or "no changes"

Push? [y/n]
```

If user says yes → push both repos:
```bash
cd $DEV/workspace/codex && git push
cd $DEV && git push  # if had changes
```

**Scope**: `$DEV` + `$DEV/workspace/codex`. For all repos including `Repos/*`, use `/commitp`.
...
- `/commit --all` - All outstanding changes in $DEV + $DEV/workspace/codex (excludes Repos/*)
- `/commit {repo}` - Specific repo only

**Scope**:
- `$DEV` + `$DEV/workspace/codex` (both repos)
