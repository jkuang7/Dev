...
class RunDocsContractTests(unittest.TestCase):
    def test_run_spec_mentions_dynamic_gate_expansion(self):
        ...
        self.assertIn("when scripted refresh fails to produce the prepared marker, controller also relaunches a fresh `/prompts:run_update ...` session", text)
        self.assertIn("after update, controller again scripts deterministic refresh itself with `runctl --setup` and `runctl --prepare-cycle`", text)
        ...
        self.assertIn("run fresh `/prompts:run_update`", text)
        self.assertIn("prefer the smallest truthful recovery", text)
        self.assertIn("treat coupling as a split trigger", text)
        self.assertIn("if a `mini` `run_update` still cannot produce the prepared marker, controller retries `run_update` on `high`", text)
    ...
    def test_run_execute_prompt_exists_and_is_execute_only(self):
        ...
        self.assertIn("It will invoke `/prompts:run_update` in a separate fresh session", text)
    ...
    def test_run_update_prompt_exists_and_is_update_only(self):
        ...
        self.assertIn("controller will run deterministic `runctl --setup` and `runctl --prepare-cycle` after this prompt exits", text)
        self.assertIn("treat `TASKS.json` as the primary backlog source of truth", text)
        self.assertIn("RUNNER_ACTIVE_BACKLOG.json", text)
        self.assertIn("do not directly edit `RUNNER_STATE.json`, `RUNNER_EXEC_CONTEXT.json`, `RUNNER_HANDOFF.md`, or `RUNNER_CYCLE_PREPARED.json`", text)
        ...
        self.assertIn("autonomous recovery prompt", text)
        self.assertIn("keep this prompt model-efficient", text)
        self.assertIn("if execute reported `scope_status=split` or `scope_status=reseed`, do not keep the same umbrella task alive unchanged", text)
        self.assertIn("do not turn ordinary hard debugging into `scope_status=reseed`", text)
        self.assertIn("prefer `scope_status=narrow` when the family boundary is still right", text)
        self.assertIn("do not merge a second coupled family into the active task during refresh", text)
        self.assertIn("reseed the active open backlog in place", text)
        self.assertIn("do not wipe all memory files", text)
        self.assertIn("this prompt is responsible for repairing the active backlog enough that the next execute cycle can continue", text)
        ...
        self.assertIn("downstream tasks with unmet `depends_on` must be `blocked`", text)
        self.assertIn("only one actionable next task should remain `open`", text)
        self.assertIn("state_repaired=<yes|no>", text)
        self.assertIn("scope_status=<ok|narrow|split|reseed>", text)
        self.assertIn("supervisor now handles runner-thread archiving itself", text)
        self.assertNotIn("keep `TASKS.json`, `RUNNER_EXEC_CONTEXT.json`, `RUNNER_STATE.json`, and `RUNNER_HANDOFF.md` aligned before exiting", text)
